...
Behavior = function (name, setup) {
  ...
  if (!name || !_.isString(name)) {
    throw new Error('Pass behaviour name as a first parameter of the `Behavior` function');
  }
  if (!setup || !_.isFunction(setup)) {
    throw new Error('Pass behaviour setup function as a second parameter of the `Behavior` function');
  }
  ...
  Behavior.prototype = new BehaviourBase();
