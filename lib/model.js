Model = function (name, schema, transform) {
  ...
  var Model = Utils.createConstructor(name, function (/* arguments */) {
    ...
    if (! (self instanceof Model)) {
      throw new Error('Use "new" to create instance of "' + name + '" model');
    }
  ...
  if (transform) {
    ...
    schema.collection._transform = Deps._makeNonreactive(transformFunction);
  } else {
