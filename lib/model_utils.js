...
ModelUtils.extendModelBase = function (modelDef) {
  var inheritDef = {};

  // Constructor
  if (_.has(modelDef, 'constructor')) {
    inheritDef.constructor = modelDef.constructor;
  }

  // Object
  inheritDef.object = {};
  if (_.has(modelDef, 'fields')) {
    _.extend(inheritDef.object, modelDef.fields);
  }
  _.each(modelDef.behaviors, function (behavior) {
    _.extend(inheritDef.object, behavior.fields);
  });
  ...
  _.extend(inheritDef.prototype, modelDef.methods);
  _.each(modelDef.behaviors, function (behavior) {
    _.extend(inheritDef.prototype, behavior.methods);
  });

  return ModelBase.extend(inheritDef);
};
